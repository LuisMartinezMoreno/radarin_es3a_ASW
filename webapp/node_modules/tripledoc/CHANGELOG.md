...
- The second argument, `locale`, to the `getAllLocaleStrings` method on TripleSubjects is now optional. If left out, it will return an array with all locale strings and their locales, as objects of the shape `{ locale: string, value: string}`. This feature was contributed by Michaud Venant — thanks!
